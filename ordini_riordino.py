...
def elabora_gestionale(df_input, data_inizio, data_fine):
   ...
   # Sostituisci la tua riga df['ID_DOCUMENTO_PADRE'] = df.apply(...) con questa:
    ...
    def classifica_famiglia(row, famiglie_aperte, famiglie_da_consultivare):
        id_fam = row['ID_FAMIGLIA']
        
        if id_fam in famiglie_aperte:
            return 'APERTO'
        elif id_fam in famiglie_da_consultivare:
            return 'CONSULTIVARE'
        else:
            return 'STANDARD'

    # 1. Identifica le famiglie APERTE (come facevi prima)
    ...
    famiglie_aperte         = set(df.loc[condizione_aperto, 'ID_FAMIGLIA'].unique())

    # 2. Identifica le famiglie DA CONSULTIVARE
    # (Es. famiglie che contengono almeno un ordine con EVASO == 'NO')
    condizione_consultivare     = (df['TIPO_DOCUMENTO'] == 'ORDINE') & (df['EVASO'] == 'NO') & (df['TIPO_DOCUMENTO_PADRE'] == 'Preventivo')
    famiglie_da_consultivare    = set(df.loc[condizione_consultivare, 'ID_FAMIGLIA'].unique())

    # 3. Applica la classificazione
    # Nota: L'uso di 'set' rende la ricerca molto più veloce rispetto a una lista su grandi DataFrame
    df['FAMIGLIA'] = df.apply(
        lambda row: classifica_famiglia(row, famiglie_aperte, famiglie_da_consultivare), 
        axis=1
    )
    ...
    def determina_status(row):
        ...
        if row.get('RIGA_SCONTO_TOTALE') == 'SI':
            return 'SCONTO'
        ...
        if tipo == 'ORDINE':
            ...
            # Controllo EVASO NO
            # Assumiamo che il campo si chiami 'EVASO'
            if row.get('EVASO') == 'NO':
                return 'DA CONSULTIVARE'
    ...
    if not orfani.empty:
        ...
        mask_recupero       = (df_input['ID_DOCUMENTI'].isin(ids_padri_mancanti))
        df_recuperato       = df_input[mask_recupero].copy()

        # 1. Recupera gli orfani e crea due mappature
        # Mappiamo il Padre (ID_DOCUMENTO_PADRE) verso i suoi attributi di famiglia
        mapping_famiglie    = orfani.set_index('ID_DOCUMENTO_PADRE')['FAMIGLIA'].to_dict()
        mapping_id_famiglie = orfani.set_index('ID_DOCUMENTO_PADRE')['ID_FAMIGLIA'].to_dict()
        
        # 2. Applichiamo entrambi al df_recuperato
        df_recuperato['FAMIGLIA']       = df_recuperato['ID_DOCUMENTI'].map(mapping_famiglie)
        df_recuperato['ID_FAMIGLIA']    = df_recuperato['ID_DOCUMENTI'].map(mapping_id_famiglie)
        df_recuperato['TIPO_DOCUMENTO'] = df_recuperato['TIPO_DOCUMENTO'].fillna('').astype(str).str.upper()
        
        # 1. Recupero degli orfani
        df = pd.concat([df, df_recuperato])
    ...
    with col2:
        ...
        soglia_scadenza = st.slider("Validità preventivi (giorni)", min_value=0, max_value=365, value=90, step=1)

    # --- 8. CONFIGURAZIONE DEI BLOCCHI DI COLONNE BASE ---
    colonne_superflue = ['SEZIONALE', 'NUMERO', 'ID_UTENTI', 'ID_AGENTI', 'ID_CLIENTI', 'FAMIGLIA', 'ID_FAMIGLIA','PADRE','ID_PADRE', 'PIVA','ATECO','PROVINCIA', 'SITO_WEB','TELEFONO','EMAIL','NAZIONE','REGIONE','CAP','INDIRIZZO','NUM_DIPE']
    client_cols = [
    ...
    if semplifica:
        ...
        colonne_ordinate = (['ID_DOC', 'ID_PADRE','PADRE','FAMIGLIA', 'TIPOLOGIA', 'STATUS', 'REV', 'DATA', 'TOTALE_RIGA', 'RIGA_SCONTO_TOTALE', 'EVASO', 'TITOLO'] + doc_cols_restanti + client_cols + ['ID_FAMIGLIA'])
    ...
    def colora_filiera_avanzato(data):
        ...
        for idx, row in data.iterrows():
            ...
            if status == 'ATTENZIONE':
                ...
                bg_color = 'background-color: #ffcccc; color: #800000;'
            ...
            # --- LOGICA ALTERNANZA PER BLOCCHI ---
                ...
                if status == 'IN ATTESA':
                    bg_color = 'background-color: #ffff99; color: #000000;' if is_pari else 'background-color: #e6e673; color: #856404;'
                
                elif status == 'SCADUTO': 
                    # Rosso pastello chiaro (pari) vs Rosso pastello scuro (dispari)
                    bg_color = 'background-color: #ff9999; color: #000000;' if is_pari else 'background-color: #ff6666; color: #800000;'

                # Logica standard (Tua originale)
                elif is_pari:
                    if tipo_doc == 'PREVENTIVO':
                        bg_color = 'background-color: #99c2ff; color: #002244;'
                    elif tipo_doc == 'ORDINE APERTO':
                        bg_color = 'background-color: #80b3ff; color: #002244;'
                    else:
                        bg_color = 'background-color: #66a3ff; color: #002244;'
                else:
                    if tipo_doc == 'PREVENTIVO':
                        bg_color = 'background-color: #90e090; color: #0b300b;'
                    elif tipo_doc == 'ORDINE APERTO':
                        bg_color = 'background-color: #77d977; color: #0b300b;'
                    else:
                        bg_color = 'background-color: #5cd65c; color: #0b300b;'
    ...
    #st.write(df_visualizzazione.head(1000))
    ...
    # 1. Identifichiamo i preventivi in attesa degli ordini standard (Famiglia STANDARD + status IN ATTESA)
    ...
    # 2. Tra quelli in attesa, quelli PERSÌ sono quelli con giorni >= soglia
    df_persi = temp_df[mask_in_attesa & (temp_df['GIORNI_TRASCORSI'] >= soglia_scadenza)].copy()
    df_persi['STATUS'] = 'SCADUTO' 
   
    # 3. Ordini CHIUSI (Famiglia STANDARD ma NON in attesa)
    ...
    # 5. SCONTI
    df_sconti = df_ordini_attivi[df_ordini_attivi['STATUS'] == 'SCONTO'].copy()

    # 6. ORDINI DA CUNSULTIVARE
    df_consultivare = temp_df[(temp_df['FAMIGLIA'] == 'CONSULTIVARE')].copy() 
    ...
    styler_sconti = df_sconti.style.apply(colora_filiera_avanzato, axis=None).format(styler_format, na_rep="")
    styler_consultivare = df_consultivare.style.apply(colora_filiera_avanzato, axis=None).format(styler_format, na_rep="")
    ...
    # Aggiungiamo un toggle per abilitare il calcolo/visualizzazione
    mostra_diagnosi = st.toggle("Abilita tabella (appesantisce il software)")  
    if mostra_diagnosi:
        ...
        with tab5:
            st.dataframe(styler_consultivare, use_container_width=True, hide_index=True, column_order=colonne_da_mostrare)
        ...
        with tab7:
            st.dataframe(styler_sconti, use_container_width=True, hide_index=True, column_order=colonne_da_mostrare)
    ...
    if 'DATA' in df_pulito.columns:
        ...
        df_consultivare = df_pulito[df_pulito['FAMIGLIA'].astype(str) == 'CONSULTIVARE'].copy()

    # Ritorna il dataframe elaborato e aggiornato
    return df_ordini, df_aperti, df_consultivare